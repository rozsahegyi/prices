...
function Prices() {
	...
	this.init = function() {
		...
		var colors = 'e01111,e0e011,11e011',
			highlight = 'Beijing,London,New York,Sydney,Tokyo,Zurich',
			set_highlights = function(x) { var res = {}; x.split(',').each(function(a) { res[a] = 1; }); return res; };
	...
	this.refresh = function() {
		...
		this.update_hash();
		this.draw_charts();
		this.debug('refresh in ' + ((new Date()).getTime() - start.getTime()) / 1000 + ' sec');
	}
	...
	this.read_hash = function() {
		...
		if (hash.length && hash[hash.length - 1].slice(0, 6) === 'order:')
			this.state.sort_index = this.int(hash.pop().slice(6));
	...
	this.update_chart_list = function(time) {
		this.state.charts = $A($('charts').childNodes).slice(1).
	...
	this.setup_chart_list = function() {
		...
		// full chart list
		var income_text = function(x) { return [x, 'Income for ' + x.replace('_', ' ')]; };
		this.chart_list = this.data.charts.concat(this.data.profession.map(income_text));
		// get active charts and sort index from the hash (or set default ones)
		this.read_hash();

		// var charts = [['', 'Show charts']].concat(this.chart_list),
		var charts = this.chart_list,
		...
		var add_chart = function(chart, i) {
			...
			el.className = this.active_charts.indexOf(el.value) > -1 ? 'active' : '';
			el.onclick = chart_onclick;
			this.list.appendChild(el);
		}

		// populate #charts with chart elements
		Prototype.K(charts).each(add_chart, { list: $('charts'), active_charts: this.state.charts });
	}

	this.draw_charts = function() {
		var m = this.state.metrics,
			bx = m.base.x, by = m.base.y,
			tx = bx + m.text.x, ty = by + m.text.y,
			width = m.chart_width,
			colormap = this.state.colormap,
			data = this.charts_data(this.state.charts),
			charts = data[0],
			fields = data[1],
			context, props, tmp;
		...
		var isnumber = function(x) { return !isNaN(parseInt(x, 10)); },
			getmax = function(list, k) { return $A(list.map(function(x) { return x[k]; })).max(); };
		...
		var display_field = function(el, i) {
			var x = i * width, y = m.name_height, text = el[2];
			if (i == Math.abs(this.state.sort_index))
				text += this.state.sort_index < 0 ? ' \u25bc' : ' \u25b2';
			this.canvas.fillStyle = this.state.background;
			this.canvas.fillRect(bx + x, by + y, m.text.x * 2 + this.canvas.measureText(text).width, m.text.y + m.stepy + 6);
			this.label(tx + x, ty + y, text);
		}
	...
	this.chart_data = function(name) {
		var i, fname, len, chart, fields = [], data = [], indexes,
			city_fields = this.data.city[0],
			city_number = this.data.city.length - 1;
	...
	this.bar = function(x, y, w, len, color, text, max, textcolor, highlighted) {
		...
		var width = this.canvas.measureText(text).width, outside = width > (len - 5);
